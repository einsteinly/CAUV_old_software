...
class MailboxTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    except ssrc.spread.Error, error:
      error._print()
      raise error
  ...
  def test_scatter_send(self):
    ...
    self.assert_(sm.add(self.message))
    self.assert_(sm.add(m))
    ...
    self.assert_(self.mbox.add_message_part(s1))
    s2 = "test_scatter_send4"
    self.assert_(self.mbox.add_message_part(s2))
    self.mbox.send()
  ...
  def test_receive_membership_message(self):
    ...
    self.assert_(info.caused_by_leave())
    self.assertEqual(MailboxTest.TestGroupName2, self.message.sender())
  ...
  def test_self_leave_message(self):
    ...
    self.assert_(info.caused_by_leave())
    self.assert_(info.is_self_leave())

  def test_error(self):
    try:
      self.mbox.join("####")
    except ssrc.spread.Error, e:
