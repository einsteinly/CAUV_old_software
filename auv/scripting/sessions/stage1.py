...
if socket.gethostname() == 'barracuda-seco':
    ...
    processes = common_processes + [
        ...
        Process('control',    '/',       node_pid('control'),      restart(), 'root', ['{BDIR}/control{D} --sbg /tmp/cauv/ports/ttyV1 --imu sbg --mcb barracuda --port /tmp/cauv/ports/ttyVCAN']),
        Process('persist',    '{SDIR}',  node_pid('persist'),      restart(),  None,  ['{SDIR}/persist.py -rs -f {SDIR}/persist/sim']),
        Process('daemon man', '{SDIR}',  node_pid('daemon_man'),   restart(),  None,  ['{SDIR}/daemon_man -peer 10.0.0.3']),
        Process('last resort','{SDIR}', search_pid('last-resort.py'),restart(),None,  ['{SDIR}/last-resort 7777 300']),
    ]
elif socket.gethostname() == 'barracuda-arm':
    # processes running on the panda board only
    processes = common_processes + [
        Process('daemon_man'  '{SDIR}',  node_pid('daemon_man'),   restart(),  None,  ['{SDIR}/daemon_man -peer 10.0.0.2']),
    ]
