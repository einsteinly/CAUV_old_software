...
class TrackWallSimple(AI.Script):
    class DefaultOptions(AI.Script.DefaultOptions):
        def __init__(self):
            ...
            self.strafeLimit = AI.OptionWithMeta(60, opt_type=MotorValue)
            self.strafeOffset = AI.OptionWithMeta(10, opt_type=MotorValue)
...
!!!! Write help here !!!!
        ...
        """
        angle, distance = self.findBestLine(m.lines)
        perpendicularRatio = self.findPerpendicularRatio(m.lines);
        ...
        #Prop Control
        speed = 0
        if perpendicularRatio < self.options.perpendicularRationLimit:
        ...
        #Bearing Control
        if self.options.controlBearing:
    ...
    def findBestLine(self, lines):
        ...
        for line in lines:
            ...
            distance += abs(t.x*a.y - t.y*a.x)*line.length
    ...
    def findPerpendicularRatio(self, lines):
        ...
        for i in range(len(lines)):
            for j in range(i,len(lines)):
                line1 = lines[i]
                line2 = lines[j]
                if abs(math.degrees(line1.angle-line2.angle)) < self.options.perpendicularError:
