...
string asCPPType(string type)
{
    map<string, string>::iterator it;
    it = cpp_conversion.find(type);
    if (it == cpp_conversion.end())
        return type;
    else
        return it->second;
}
string asCPPType(Type* type)
{
    ...
    {
        case TYPE_BASE:
            return asCPPType(static_cast<BaseType*>(type)->getName());
        case TYPE_LIST:
        {
...
int main(int argc, char** argv)
{
    ...
    po::options_description desc("USAGE:  msg-generator [-o FILENAME] [INPUT]");
    desc.add_options()
    ...
    string inputpath = "-", outputpath = "messages"; 
    if (vm.count("input"))
